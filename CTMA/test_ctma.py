...
def test_save_data_success(load_mock_tasks, tmp_path):
    """
    Verifies th.saveData creates the correct JSON structure.
    ...
    with patch.object(th, "open", mock_file):
        th.saveData()

    mock_file.assert_called_once_with("data.json", "w")

    written_data = "".join(
        call[1][0] for call in mock_file.mock_calls if call[0] == "().write"
    )
...
def test_edit_task_category_handler_success(load_mock_tasks):
    ...
    success = c.update_task_attributes(
        task_id,
...
def test_edit_task_attribute_failure_invalid_id(load_mock_tasks):
    ...
    """
    success = c.update_task_attributes(999, "Fail", "", "1", "Fail")
